...
const ReqforRent = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      ...
      try {
        // Query to fetch requests based on form_type == "Renter" and listerUid
        const q = query(
          collection(db, "renter_requests"),
          where("listerUid", "==", user.uid),
          where("form_type", "==", "Renter")
        );
  ...
  const updateStatus = async (id, status) => {
    try {
      const ref = doc(db, "renter_requests", id);
  ...
  return (
    <div className="max-w-6xl mx-auto mt-6 px-4">
      <h2 className="text-2xl font-semibold text-gray-800 mb-6">
        Renter Requests for Your Listings
      </h2>
